...
authApp.post('/register', async (req, res) => {
  ...
  if (
    (await model_individual_users_1.ModelIndividualUser.collection().findOne({
      phone,
    })) != undefined
  ) {
  ...
  try {
    ...
    await model_otp_1.ModelOtp.collection().updateOne(
      { phone },
      {
        $set: {
...
authApp.post('/login', async (req, res) => {
  ...
  //     await model_individual_users_1.ModelIndividualUser.collection().updateOne(
  //       { id: user.id },
  //       {
  //         $set: {
  //           status: 'active',
